...
var _charming = __webpack_require__(4);
...
*/
{
  ...
  var Name = function () {
    ...
    _createClass(Name, [{
      ...
      value: function init() {
        ...
        (0, _charming2.default)(this.el, {
          classPrefix: 'letter'
        });

        // Create new array to store a new virtual
        // object (Letter) reference for each letter in Name
        var letters = [];
        ...
        var letterAnimation = Object.assign(targets, this.animations.letters.show);

        // Animate after timeout/this.animations.delay
        this.playAnimation(letterAnimation);
      }
    }, {
      key: 'playAnimation',
      value: function playAnimation(letterAnimation) {
        setTimeout(function () {

          _animejs2.default.timeline().add(letterAnimation);
        }, this.animations.delay);
      }
      // createShapes(scene, shapesPerLetter){
      ...
      this.animations = {
        shapes: {
          ...
          translateY: function translateY() {
            return [_animejs2.default.random(-15, 15), _animejs2.default.random(-200, 200)];
          },
  ...
  var Shape = function () {
    function Shape(letter, letterProps) {
      _classCallCheck(this, Shape);

      this.el = undefined;
      this.parent = document.querySelector('.shapes');
      this.x = _animejs2.default.random(letterProps.x, letterProps.x + letterProps.width);
      this.y = _animejs2.default.random(letterProps.y - letterProps.height, letterProps.y);
      this.scale = _animejs2.default.random(letterProps.width * .1, letterProps.width * .75);
      this.colors = ['#4cabef', '#fff473', '#bf2525'];
      this.types = [{
        el: 'circle',
        cx: this.x,
        cy: this.y,
